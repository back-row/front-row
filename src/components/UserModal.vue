  ...
  >
    <div class="flex">
      <div class="flex flex-col items-center justify-center mx-4">
        <div class="flex">
          <h2 v-show="!editNameVisible" class="text-whiteBackRow text-2xl mt-4">{{ userStore.user.name }}</h2>
          <input v-show="editNameVisible" class="w-40 h-6 mt-4" v-model="userStore.user.name" type="text" placeholder="New username" @keydown.enter="saveName" required/>
          <img class="w-6 h-6 my-4 ml-2 cursor-pointer" :src="'src/assets/pen.png'" @click="toggleEditName"/>
        </div>
        <div class="flex">
          <h4 v-show="!editEmailVisible" class="text-whiteBackRow text-lg">{{ userStore.user.email }}</h4>
          <input v-show="editEmailVisible" class="w-40 h-5" v-model="userStore.user.email" type="text" placeholder="New username" @keydown.enter="saveEmail" required/>
          <img class="w-6 h-6 ml-2 cursor-pointer" :src="'src/assets/pen.png'"  @click="toggleEditEmail"/>
        </div>
        <h2 class="text-whiteBackRow text-xl mt-8">Level</h2>
        <p class="text-whiteBackRow text-2xl">{{ userStore.user.level }}</p>
      ...
      <div class="flex flex-col">
        ...
        <div class="flex flex-col ml-2 w-28">
          <div v-for="avatar in avatars" :key="avatar.src">
            <input type="radio" class='accent-greenBackRow' :value="avatar.src" v-model="userStore.user.avatar" @change="saveUser"/>
    ...
    <div class="flex">
      <button @click="logout" class="bg-greenBackRow text-whiteBackRow rounded-md p-2 mb-4">
        Logout
      </button>
    </div>
  </div>
